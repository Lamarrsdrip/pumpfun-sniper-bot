...
export default function TokenDetail() {
  ...
  return <ScrollView style={styles.root} contentContainerStyle={styles.content}>
    <View style={styles.hero}><View style={{ flex: 1 }}><Text style={styles.name}>{token.name}</Text><Text style={styles.symbol}>{token.symbol} · {token.chain.replaceAll('_', ' ')} · {token.source}</Text></View><View style={styles.score}><Text style={styles.scoreValue}>{token.runnerScore}</Text><Text style={styles.scoreLabel}>Runner</Text></View></View>
    ...
    <Card tone="warning"><Text style={styles.riskTitle}>Highest risk</Text><Text style={styles.summary}>{data.explanation.highestRisk}</Text>{token.contractWarnings.length ? <Text style={styles.disclaimer}>Contract warnings: {token.contractWarnings.join(' · ')}</Text> : null}</Card>
    ...
    <View style={{ flexDirection: 'row', gap: 8 }}><Pressable onPress={() => Linking.openURL(token.sourceMode === 'EARLY_SOLANA' ? `https://pump.fun/coin/${token.mint}` : `https://dexscreener.com/search?q=${encodeURIComponent(token.mint)}`)} style={{ flex: 1, padding: 12, borderRadius: 8, backgroundColor: dark.surface, alignItems: 'center' }}><Text style={{ color: dark.text, fontWeight: '800' }}>{token.sourceMode === 'EARLY_SOLANA' ? 'Open Pump.fun' : 'Open DEX Screener'}</Text></Pressable><Pressable onPress={() => router.push('/swap')} style={{ flex: 1, padding: 12, borderRadius: 8, backgroundColor: dark.surface, alignItems: 'center' }}><Text style={{ color: dark.text, fontWeight: '800' }}>Swap</Text></Pressable></View>
