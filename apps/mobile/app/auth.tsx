...
const slides = [
  { tag: 'WELCOME TO NAIRAMEME', title: 'Your Naira gateway to meme markets.', body: 'Trade memes, swap crypto, earn rewards, and withdraw to Nigerian banks.', color: '#20D68F' },
  ...
  { tag: 'EARN', title: 'Turn your skills and alpha into rewards.', body: 'Join bounties, campaigns, and risk-adjusted competitions.', color: '#F6C85F' }
];
export default function AuthScreen() {
  ...
  const enterDemo = async () => {
    ...
    try {
      ...
      // Demo content is bundled for review, so a temporary preview tunnel must not block entry.
      await saveSession('offline-demo-preview', 'DEMO');
      router.replace('/(tabs)');
    } finally {
      setBusy(false);
    }
  };
  if (step < slides.length) { const slide = slides[step]; return <LinearGradient colors={['#070B0A', `${slide.color}17`, '#070B0A']} style={{ flex: 1, padding: 24, justifyContent: 'space-between' }}><View style={{ paddingTop: 70 }}><View style={{ width: 62, height: 62, borderRadius: 20, backgroundColor: slide.color, alignItems: 'center', justifyContent: 'center' }}><Text style={{ color: '#06110D', fontSize: 20, fontWeight: '900' }}>NM</Text></View><Text style={{ color: slide.color, fontSize: 12, fontWeight: '900', marginTop: 34 }}>{slide.tag}</Text><Text style={{ color: dark.text, fontSize: 40, lineHeight: 44, fontWeight: '900', marginTop: 10 }}>{slide.title}</Text><Text style={{ color: dark.muted, fontSize: 16, lineHeight: 24, marginTop: 16 }}>{slide.body}</Text></View><View style={{ gap: 14 }}><View style={{ flexDirection: 'row', gap: 6 }}>{slides.map((_, i) => <View key={i} style={{ height: 4, flex: 1, borderRadius: 2, backgroundColor: i <= step ? slide.color : dark.border }} />)}</View><Button title={step === 3 ? 'Get started' : 'Continue'} onPress={() => setStep(step + 1)} /><Pressable onPress={() => setStep(4)}><Text style={{ color: dark.muted, textAlign: 'center', padding: 6 }}>Skip introduction</Text></Pressable></View></LinearGradient>; }
  return <Page><View style={{ paddingTop: 24 }}><Text style={{ color: dark.green, fontWeight: '900' }}>NAIRAMEME</Text><Text style={{ color: dark.text, fontSize: 34, fontWeight: '900', marginTop: 8 }}>Welcome back</Text><Text style={{ color: dark.muted, marginTop: 7 }}>Continue with phone or email.</Text></View><TextInput value={identifier} onChangeText={setIdentifier} placeholder="Phone number or email" placeholderTextColor={dark.muted} style={{ minHeight: 54, color: dark.text, backgroundColor: dark.surface, borderColor: dark.border, borderWidth: 1, borderRadius: 12, paddingHorizontal: 14 }} />{error ? <Text style={{ color: dark.red }}>{error}</Text> : null}<Button title="Continue securely" disabled={identifier.trim().length < 5} onPress={() => router.push('/kyc')} /><Button title={busy ? 'Opening demo...' : 'Explore Demo Mode'} kind="secondary" disabled={busy} onPress={enterDemo} /><Text style={{ color: dark.yellow, fontSize: 11, lineHeight: 17 }}>Demo Mode is clearly labelled sample money and simulated execution. Live mode never uses invented balances, tokens, or transactions.</Text></Page>;
}
