...
export default function ProfileScreen() {
  ...
  useEffect(() => { api<{ isAdmin: boolean }>('/v1/me').then((value) => setIsAdmin(value.isAdmin)).catch(() => setIsAdmin(false)); }, []);
  return <Page><View style={{ flexDirection: 'row', gap: 13, alignItems: 'center' }}><AssetIcon symbol="AN" size={58} /><View><Text style={{ color: dark.text, fontSize: 23, fontWeight: '900' }}>Ada Nwosu</Text><Text style={{ color: dark.muted }}>ada@demo.nairameme.ng</Text></View></View><ModePill />
    ...
    <Button title="Open Meme Network" kind="secondary" onPress={() => router.push('/network')} />{isAdmin ? <Button title="Open Admin Mode" kind="secondary" onPress={() => router.push('/admin')} /> : null}
