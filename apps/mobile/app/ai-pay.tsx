...
export default function AiPayScreen() {
  ...
  const prepare = async () => { setBusy(true); setMessage(''); try { setDraft((await api<{ payment: Draft }>('/v1/ai-pay/prepare', { method: 'POST', body: JSON.stringify({ instruction, source: 'TEXT' }) })).payment); } catch (error) { setMessage(error instanceof ApiError ? error.message : 'Payment details could not be prepared.'); } finally { setBusy(false); } };
  ...
  return <Page>
    ...
    {!draft ? <Card>
      ...
      <Button title="Prepare payment" busy={busy} disabled={instruction.trim().length < 10} onPress={prepare} icon="sparkles" />
      ...
      <Detail label="Recipient" value={draft.accountName} /><Detail label="Bank" value={draft.bankName} /><Detail label="Account number" value={draft.accountNumber} /><Detail label="Amount" value={formatNaira(amount)} /><Detail label="Fee" value={formatNaira(fee)} /><Detail label="Balance after" value={formatNaira(318450 - amount - fee)} /><Detail label="Narration" value={draft.narration} />
