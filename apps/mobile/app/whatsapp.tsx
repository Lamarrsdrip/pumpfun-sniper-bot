...
export default function WhatsappScreen() {
  ...
  return <Page>
    ...
    <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>{['balance', 'account number', 'recent transactions', 'savings status', 'pending approvals', 'orders', 'pause auto pay', 'prepare payment'].map((item) => <Pressable key={item} onPress={() => setCommand(item)} style={{ paddingHorizontal: 11, paddingVertical: 8, borderRadius: 99, backgroundColor: dark.surface, borderWidth: 1, borderColor: dark.border }}><Text style={{ color: dark.textSoft, fontSize: 10, fontWeight: '800' }}>{item}</Text></Pressable>)}</View>
